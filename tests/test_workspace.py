...
def test_the_audit_page_totals_every_run(client):
    ...
    assert "under the runs folder" not in body
    assert "workspace.db" not in body
...
def test_an_empty_workspace_offers_the_pack_on_the_page_it_can_reach(empty_workspace):
    """Without an objective record the workspace still renders.

    Files arrive from the browser, or from the demo pack. Neither requires a
    committed data folder, and neither requires a text editor.
    """
    http, settings = empty_workspace
    ...
    assert "Clear uploaded files" not in body
    assert 'class="filerow"' not in body
    assert 'class="filelist"' not in body
