...
def test_new_run_explains_a_missing_data_folder(client, tmp_path, monkeypatch):
    """An empty folder is a starting point, not a 500. Files come from the browser."""
    ...
    assert "Drop the quarter's files here" in response.text
    assert 'id="filepick"' in response.text
    assert 'action="/evidence/demo"' in response.text
...
def test_the_audit_trail_records_stages_model_calls_and_edits(client):
    ...
    assert "How much changed" in body
...
def test_change_label_is_readable():
    from app.audit import change_label

    assert change_label({"edit_kind": "acknowledged", "char_distance": 1}) == "—"
    assert change_label({"edit_kind": "acknowledgement_cleared", "char_distance": 1}) == "—"
    assert change_label({"edit_kind": "value_change", "char_distance": 0}) == "Unchanged"
    assert change_label({"edit_kind": "value_change", "char_distance": 1}) == "1 character"
    assert change_label({"edit_kind": "value_change", "char_distance": 24}) == "24 characters"
    assert change_label({"edit_kind": "value_change", "char_distance": None}) == "—"
...
def test_a_file_larger_than_one_megabyte_is_accepted(client, data_dir):
    """Starlette's default multipart cap is 1 MB. A long export used to die there."""
    http, _ = client
    payload = ("# Long export\n\n" + ("word " * 250_000)).encode()
    assert len(payload) > 1_000_000
    response = http.post(
        "/evidence/add",
        files={"files": ("long.md", payload, "text/markdown")},
    )
    assert "Added" in response.text
    assert (data_dir / "evidence" / "long.md").exists()
    assert (data_dir / "evidence" / "long.md").stat().st_size > 1_000_000


def test_several_uploads_are_accepted_together(client, data_dir):
    http, _ = client
    response = http.post(
        "/evidence/add",
        files=[
            ("files", ("note-a.md", "# Note A — 3 May 2026\n\nFirst.".encode(), "text/markdown")),
            ("files", ("note-b.md", "# Note B — 4 May 2026\n\nSecond.".encode(), "text/markdown")),
            ("files", ("note-c.txt", b"A plain text note.", "text/plain")),
        ],
    )
...
def test_uploaded_files_can_be_cleared(client, data_dir):
    http, _ = client
    body = http.get("/runs/new").text
    assert 'action="/evidence/clear"' in body
    assert "Clear uploaded files" in body

    response = http.post("/evidence/clear")
    assert "Cleared" in response.text
    assert not (data_dir / "objective.md").exists()
    assert not (data_dir / "prior_update.md").exists()
    assert list((data_dir / "evidence").glob("*.md")) == []
    assert "Clear uploaded files" not in http.get("/runs/new").text


def test_a_cleared_workspace_can_be_filled_from_the_browser(client, data_dir):
    http, _ = client
    http.post("/evidence/clear")
    http.post(
        "/evidence/add",
        files={"files": ("fresh.md", "# Fresh — 8 August 2026\n\nFrom the browser.".encode(), "text/markdown")},
    )
    page = http.get("/runs/new").text
    assert "fresh.md" in page
    assert not (data_dir / "objective.md").exists()
    assert (data_dir / "evidence" / "fresh.md").exists()
