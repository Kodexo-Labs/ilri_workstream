...
def change_label(edit: dict | None) -> str:
    """Size of an edit in words a director can read without a legend.

    Acknowledgements are not wording changes, so they are a dash rather than
    a one-character distance on the bookkeeping string. A rewrite is counted
    in characters added, removed or replaced.
    """
    if not edit:
        return "—"
    kind = edit.get("edit_kind") or ""
    if kind in {"acknowledged", "acknowledgement_cleared"}:
        return "—"
