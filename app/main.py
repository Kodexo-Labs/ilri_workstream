...
from .inputs import load_inputs, load_workspace
...
templates.env.filters["change_label"] = audit.change_label
...
def workspace_context() -> dict:
    """What the setup screen shows, whether or not anyone has uploaded yet.

    The pipeline still refuses to run without an objective record. This page
    must not: that is how the files get here, instead of being compiled in.
    """
...
async def new_run(request: Request):
    return render(request, "new.html", workspace_context(), nav="new")
...
async def add_evidence(request: Request):
    """Accept as many files as the browser sends, into the data folder.

    Starlette's multipart parser defaults to 1 MB per part and that used to
    refuse a drop of several notes before the store ran its own check. The
    limits here are the store's, so a director can keep adding files until
    the run is ready rather than until a framework default says stop.
    """
    form = await request.form(
        max_files=store.MAX_UPLOAD_FILES,
        max_fields=20,
        max_part_size=store.MAX_UPLOAD_BYTES,
    )
    added: list[str] = []
    skipped: list[str] = []
    try:
        for item in form.getlist("files"):
            ...
            content = await read() if read else b""
            try:
                path = store.add_uploaded(settings.evidence_dir, filename, content)
        ...
        if not added:
            raise store.StoreError(
                skipped[0]
                if skipped
                else "Nothing to add — drop some files or choose them with browse."
            )
    except store.StoreError as error:
        return _workspace(request, error=str(error))

    message = f"Added {added[0]}." if len(added) == 1 else "All docs uploaded."
    if skipped:
        return _workspace(
            request,
            message=message,
            error="Skipped: " + " ".join(skipped),
        )
    return _workspace(request, message=message)
...
@app.post("/evidence/clear")
async def clear_evidence(request: Request):
    """Empty the input folder. Draft runs are left alone."""
    try:
        removed = store.clear_workspace(settings)
    except OSError as error:
        return _workspace(request, error=f"Could not clear the files: {error}")
    if not removed:
        return _workspace(request, message="Nothing to clear.")
    return _workspace(
        request,
        message=f"Cleared {len(removed)} files. Upload your own, or load the demo pack.",
    )
...
async def delete_evidence(request: Request, filename: str):
    try:
        store.remove_workspace(settings, filename)
    except store.StoreError as error:
        return _workspace(request, error=str(error))
...
def _workspace(request: Request, message: str = "", error: str = ""):
    """Re-render the new-run workspace, including when it is empty."""
    return render(
        request,
        "new.html",
...
async def create_run(
    ...
    try:
        ...
        return _workspace(
            request,
            error="There is no objective record. Upload one and mark it as the Objective record, or load the demo pack.",
        )
    if not inputs["docs"]:
...
async def audit_markdown(thread_id: str):
    ...
    if ledgers["edits"]:
        ...
        for edit in ledgers["edits"]:
            distance = audit.change_label(edit)
