...
(function () {
  ...
  /* ---- the dropzone ----------------------------------------------------
   *
   * Clicking opens the file picker. Dropping copies the FileList onto the
   * input through DataTransfer — assigning dataTransfer.files directly throws
   * in Chromium, which used to swallow the drop and upload nothing.
   */

  function assignFiles(input, files) {
    if (!input || !files || !files.length) return false;
    if (typeof DataTransfer === "undefined") {
      try { input.files = files; return input.files && input.files.length > 0; }
      catch (e) { return false; }
    }
    var dt = new DataTransfer();
    for (var i = 0; i < files.length; i++) {
      if (files[i] && files[i].name) dt.items.add(files[i]);
    }
  ...
  function wireDropzone() {
    ...
    input.addEventListener("change", function () { submitDropzone(); });
    ["dragenter", "dragover"].forEach(function (n) {
      zone.addEventListener(n, function (e) { e.preventDefault(); e.stopPropagation(); zone.classList.add("drag"); });
    });
    zone.addEventListener("dragleave", function (e) { e.preventDefault(); zone.classList.remove("drag"); });
    zone.addEventListener("drop", function (e) {
      ...
      if (!assignFiles(input, files)) {
        toast("Could not attach those files — try Browse instead", true);
        return;
      }
      submitDropzone();
    });
  ...
  document.addEventListener("click", function (event) {
    ...
    switch (action) {
      ...
      case "cancel-override":
        ...
        break;
      case "pick-files":
        if (event.target.closest("button") || event.target.id === "filepick") break;
        $("#filepick").click();
        break;
      case "dismiss-cite": {
        event.preventDefault();
